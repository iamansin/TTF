...
save_risk_metrics(portfolio, stocks, weights_dict, returns, riskfree=0.021, values=values, 
                  max_drawdown=max_drawdown, portfolio_returns=portfolio_returns, 
                  portfolio_sd=portfolio_sd, tickers=tickers)
...
fig1 = px.bar(risk, x="weight", y='items', color=risk.index, orientation='h', barmode='stack', text_auto='.0%',
             height=250,
             color_discrete_sequence=px.colors.qualitative.G10,
             title='Portfolio Distribution')
fig1.update_yaxes(visible=False, showticklabels=False)
fig1.update_xaxes(visible=False, showticklabels=False)
fig1.update_layout(legend=dict(orientation="h", yanchor="bottom",y=-0.7,xanchor="right",x=0.7), margin=dict(t=40, b=100)) 

# ----- To show risk distribution
fig2 = px.bar(risk, x="norm_vol", y=risk['items'], color=risk.index, orientation='h', barmode='stack', text_auto='.0%',
             height=250,
             color_discrete_sequence=px.colors.qualitative.G10,
             title='Risk Distribution')
fig2.update_yaxes(visible=False, showticklabels=False)
fig2.update_xaxes(visible=False, showticklabels=False)
fig2.update_layout(legend=dict(orientation="h", yanchor="bottom",y=-0.7,xanchor="right",x=0.7), margin=dict(t=40, b=100)) 
...
with tab1:
      ...
      with col3:
            ...
            st.write("It is calculated by dividing excess return by portfolio's overall risk")
...
with tab2:
      ...
      downside_deviation = np.sqrt(np.dot(np.array(weights).T, np.dot(downside_returns.cov(), np.array(weights)))) * np.sqrt(252)
      ...
      with col3:
            st.write("The **Sortino Ratio** is a variation of Sharpe Ratio, that only accounts for downside risk")
            st.write("A higher Sortino Ratio indicates better risk-adjusted performance.")
            st.write("if Sortino > Sharpe, portfolio volatilty is more towards positive returns (Optimal Case)")
            st.write("if Sortino < Sharpe, portfolio has significant negative volatilty Really Bad)")
with tab3:
      query = "Maximum Drawdown"
      col1, col2, col3 = st.columns([0.2,1,3])
      with col2:
            st.metric(query, value = str(round(max_drawdown*100, 2)) + "%", delta = round(max_drawdown_value, 2))
      with col3:
            st.write("The **Maximum Drawdown (MDD)** measures the largest peak-to-trough decline in the value of a portfolio during a specific period.")
            st.write("It helps assess the hypothetical risk of severe losses in a portfolio's value.")
            st.write("Lower the Maximum Drawdown, lower the risk exposure to losses")
            st.write("A high MDD (25%+) indicates higher risk, portfolio has experienced significant volatility.")
            st.write("A low MDD (10% to 15%) indicates lower risk, because of stable price changes.")
...
with tab4:
      ...
      col1, col2, col3 = st.columns([0.2,1,3])
      with col2:
            st.metric("10-day Value at Risk (at .95)", value = str(round(var_relative*100,2))+"%", delta = round(-var_absolute,0))
      with col3:
            st.write("**Value at Risk (VaR)** estimates the potential maximum loss in a portfolio over a specified time period with a given confidence level.")
            st.write("It is widely used to gauge the risk of loss on a portfolio's investments.")
            st.write("A lower VaR indicates lower potential losses under normal market conditions (Indicates Lower Risk).")
            st.write("A higher VaR signals the portfolio is exposed to larger potential losses (Indicates Higher Risk).")
            st.write("Note that VaR does not account for extreme tail risks or unusual market conditions.")
